...
def CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed):
    for row in game:
        if row.count(turn_pieces[1-turn][-2]) == 1:
            enemy_kingpos[0] = game.index(row)
            enemy_kingpos[1] = row.index(turn_pieces[1-turn][-2])
            break # saves time #
    if enemy_kingpos[0] == 0 and 0 < enemy_kingpos[1] < 7:
        pass
    # Can use below?
...
def PlayChess():
    ...
    while True:
        ...
        if len(piece) == 2: # ------------- PAWN MOVEMENT ------------- #
                ...
                CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed)
