...
def MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,capture,promo,enemy_kingpos,movesplayed,event):
    ...
    turn = 1 - turn
    event = CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
    return turn, event

def PawnMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,en_passant,ep_square,enemy_kingpos,movesplayed,event):
    ...
    if c_piece - c_move == 0:
        if side*(r_piece - r_move) > 2:
            ...
            if side*(r_piece - r_move) == 2:
                if game[r_piece+(1*other_side)][c_piece] != '-' or game[r_move][c_move] != '-':
                    ...
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,0,0,enemy_kingpos,movesplayed,event)
            ...
            if game[r_move][c_move] != '-':
                ...
                if r_move == starting_square[1-turn]-(1*side):
                    ...
                    while promo not in turn_pieces[turn] or promo == turn_pieces[turn][-2] or promo == turn_pieces[turn][-1]:
                        promo = input("Promote to: ")
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,promo,0,1,enemy_kingpos,movesplayed,event)
                else:
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,0,0,enemy_kingpos,movesplayed,event)
    else:
        if side*(r_piece - r_move) < 0:
            ...
            if movesplayed[-1] != ep_square:
                ...
                turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,1,0,enemy_kingpos,movesplayed,event)
            ...
            while promo not in turn_pieces[turn] or promo == turn_pieces[turn][-2] or promo == turn_pieces[turn][-1]:
                promo = input("Promote to: ")
            turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,promo,1,1,enemy_kingpos,movesplayed,event)
...
def MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event):
    if game[r_move][c_move] == '-':
        game[r_piece][c_piece] = '-'
...
def RookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,enemy_kingpos,movesplayed,event):
    rook1, rook2 = ['a1','a8'], ['h1','h8']
    if LegalRookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
...
def KnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event):
    if LegalKnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
    return turn, event

def BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event):
    if LegalBishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
    return turn, event

def QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,enemy_kingpos,movesplayed,event):
    if (r_move - r_piece == 0 and c_move - c_piece != 0) or (r_move - r_piece != 0 and c_move - c_piece == 0):
        turn, rook1moves, rook2moves, event = RookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,enemy_kingpos,movesplayed,event)
    elif abs(r_move - r_piece) == abs(c_move - c_piece):
        turn, event = BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
    else:
        print(f'\nIllegal move. The {all_pieces[piece[0]]} cannot move like that.')
    return turn, event

def KingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,kingmoves,rook1moves,rook2moves,enemy_kingpos,movesplayed,event):
...
def CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event):
    for row in game:
        if row.count(turn_pieces[turn][-2]) == 1:
            ...
            enemy_kingpos[1] = row.index(turn_pieces[turn][-2])
            break
...
##    game = CreateGame()
    ...
    kingmoves, rook1moves, rook2moves, enemy_kingpos = TurnItems()
    while True:
        ...
        if len(piece) == 2: # ------------- PAWN MOVEMENT ------------- #
                ...
                if game[r_piece][c_piece] not in turn_pieces[turn]:
                    ...
                    continue
                turn, en_passant, ep_square, event = PawnMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,en_passant,ep_square,enemy_kingpos,movesplayed,event)
            ...
            if piece[0] not in turn_pieces[turn]:
                ...
                turn,event = KnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][2]: # ------------- BISHOP MOVEMENT ------------- #
                turn,event = BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][3]: # ------------- QUEEN MOVEMENT ------------- #
                turn,event = QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][4]: # ------------- KING MOVEMENT ------------- #
                turn,kingmoves,event = KingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,kingmoves,rook1moves,rook2moves,enemy_kingpos,movesplayed,event)
