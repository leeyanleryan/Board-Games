...
def PawnMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,en_passant,ep_square,movesplayed):
    ...
    if c_piece - c_move == 0:
        if side*(r_piece - r_move) > 2:
            ...
            if side*(r_piece - r_move) == 2: # HOME SQUARE #
                if game[r_piece+(1*other_side)][c_piece] != '-' or game[r_move][c_move] != '-':
                    ...
                    if c_move-1 < 0 and game[r_move][c_move+1] == other_pawn: # CHECK EN PASSANT CONDITION #
                        en_passant = c_move
                        ep_square = move
                    elif c_move+1 > 7 and game[r_move][c_move-1] == other_pawn:
                        en_passant = c_move
                        ep_square = move
                    elif game[r_move][c_move+1] == other_pawn or game[r_move][c_move-1] == other_pawn:
                        en_passant = c_move
                        ep_square = move
            if side*(r_piece - r_move) == 1: # OTHER SQUARES #
            ...
            if game[r_move][c_move] != '-':
                ...
                if r_move == starting_square[1-turn]-(1*side): # PROMOTION  #
        ...
        elif en_passant != -1 and c_move == en_passant and game[r_move+(1*side)][c_move] == other_pawn: # EN PASSANT #
        ...
        elif r_move == starting_square[1-turn]-(1*side) and game[r_move][c_move] in turn_pieces[1-turn] and game[r_move][c_move] != turn_pieces[1-turn][-2]: # CAPTURE PROMO #
            promo = input("Promote to: ")
            while promo not in turn_pieces[turn] or promo == turn_pieces[turn][-2] or promo == turn_pieces[turn][-1]:
                promo = input("Promote to: ")
...
def QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed):
    if r_move - r_piece == 0 and c_move - c_piece != 0:
        turn = RookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed)
...
def KingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,kingmoves,rook1moves,rook2moves,movesplayed):
    ...
    if r_move - r_piece >= -1 and r_move - r_piece <= 1 and c_move - c_piece >= -1 and c_move - c_piece <= 1:
        ...
        if game[side[turn]][4] != all_pieces_flipped['King'][turn]:
            print(f'\nIllegal move. King cannot move like that.')
        elif kingmoves[turn] != 0:
            print(f'\nIllegal move. King has already moved.')
        elif rook2moves[turn] != 0:
            print(f'\nIllegal move. Rook has already moved.')
            ...
            print(f'\nIllegal move. Cannot castle as there are pieces in the way.')
    elif r_move - r_piece == 0 and c_move - c_piece == -2:
        if game[side[turn]][4] != all_pieces_flipped['King'][turn]:
            print(f'\nIllegal move. King cannot move like that.')
        elif kingmoves != 0:
    ...
    while True:
        ...
        if len(piece) == 2: # ------------- PAWN MOVEMENT ------------- #
            ...
            if piece[0] not in turn_pieces[turn]:
                ...
                turn = QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed)
