...
movesplayed_events = ['','+','#','1/2-1/2','1/2+1/2']
...
def GetKingPosition(game,turn,current_kingpos,enemy_kingpos):
    no_currentking = 1
    no_enemyking = 1
    for row in game:
        if row.count(turn_pieces[turn][-2]) == 1:
            no_currentking -= 1
            current_kingpos[0] = game.index(row)
            current_kingpos[1] = row.index(turn_pieces[turn][-2])
            break
    for row in game:
        if row.count(turn_pieces[1-turn][-2]) == 1:
            no_enemyking -= 1
            enemy_kingpos[0] = game.index(row)
            enemy_kingpos[1] = row.index(turn_pieces[1-turn][-2])
            break
    if no_currentking == 1:
        current_kingpos[0] = 0
        current_kingpos[0] = 0
...
def MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,capture,promo,current_kingpos,enemy_kingpos,movesplayed,event):
    if IsPinned(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,event) == 1:
        return turn, event
    ...
    if capture == 0 and promo == 0:
        ...
        movesplayed.append(f'{piece[0]}x{move}={pawn}')
    turn = 1 - turn
    if CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event) == '+':
        movesplayed[-1] += '+'
        event = '+'
    else:
        event = 'none'
...
def PawnMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,en_passant,ep_square,current_kingpos,enemy_kingpos,movesplayed,event):
    ...
    if c_piece - c_move == 0:
        if side*(r_piece - r_move) > 2:
            ...
            if side*(r_piece - r_move) == 2:
                if game[r_piece+(1*other_side)][c_piece] != '-' or game[r_move][c_move] != '-':
                    ...
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,0,0,current_kingpos,enemy_kingpos,movesplayed,event)
            ...
            if game[r_move][c_move] != '-':
                ...
                if r_move == starting_square[1-turn]-(1*side):
                    ...
                    while promo not in turn_pieces[turn] or promo == turn_pieces[turn][-2] or promo == turn_pieces[turn][-1]:
                        promo = input("Promote to: ")
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,promo,0,1,current_kingpos,enemy_kingpos,movesplayed,event)
                else:
                    turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,0,0,current_kingpos,enemy_kingpos,movesplayed,event)
    else:
        if side*(r_piece - r_move) < 0:
            ...
            if movesplayed[-1] != ep_square:
                ...
                turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,1,0,current_kingpos,enemy_kingpos,movesplayed,event)
            ...
            while promo not in turn_pieces[turn] or promo == turn_pieces[turn][-2] or promo == turn_pieces[turn][-1]:
                promo = input("Promote to: ")
            turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,promo,1,1,current_kingpos,enemy_kingpos,movesplayed,event)
            ...
            turn, event = MovePawn(game,turn,piece,r_piece,c_piece,move,r_move,c_move,pawn,1,0,current_kingpos,enemy_kingpos,movesplayed,event)
    return turn, en_passant, ep_square, event

def MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event):
    if piece[0] != 'K' and piece[0] != 'k' and IsPinned(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,event) == 1:
        return turn,event
    if game[r_move][c_move] == '-':
        game[r_piece][c_piece] = '-'
...
def ShortCastles(game,turn,piece,r_piece,c_piece,move,r_move,c_move,side,kingmoves,rook2moves,enemy_kingpos,movesplayed,event):
    ...
    elif game[r_piece][c_piece+1] == '-' and game[r_piece][c_piece+2] == '-' and game[r_piece][c_piece+3] == all_pieces_flipped['Rook'][turn]:
        game[r_piece][c_piece] = '-'
...
def LongCastles(game,turn,piece,r_piece,c_piece,move,r_move,c_move,side,kingmoves,rook1moves,enemy_kingpos,movesplayed,event):
    if game[side[turn]][4] != all_pieces_flipped['King'][turn]:
        print(f'\nIllegal move. The King cannot move like that.')
    elif kingmoves[turn] != 0:
        print(f'\nIllegal move. The King has already moved.')
    ...
    elif game[r_piece][c_piece-1] == '-' and game[r_piece][c_piece-2] == '-' and game[r_piece][c_piece-3] == '-' and game[r_piece][c_piece-4] == all_pieces_flipped['Rook'][turn]:
        game[r_piece][c_piece] = '-'
...
def LegalKnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed):
...
def LegalKingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,event):
    if r_piece == current_kingpos[0] and c_piece == current_kingpos[1]:
        game[r_piece][c_piece] = '-'
        if CheckRookQueen(game,turn,r_move,c_move,event) == '+':
            print(f'\nIllegal move. There is an enemy piece checking that spot.')
            game[r_piece][c_piece] = turn_pieces[turn][-2]
            return 0
        elif CheckKnight(game,turn,r_move,c_move,event) == '+':
            print(f'\nIllegal move. There is an enemy piece checking that spot.')
            game[r_piece][c_piece] = turn_pieces[turn][-2]
            return 0
        elif CheckBishopQueen(game,turn,r_move,c_move,event) == '+':
            print(f'\nIllegal move. There is an enemy piece checking that spot.')
            game[r_piece][c_piece] = turn_pieces[turn][-2]
            return 0
        elif CheckPawn(game,turn,r_move,c_move,event) == '+':
            print(f'\nIllegal move. There is an enemy piece checking that spot.')
            game[r_piece][c_piece] = turn_pieces[turn][-2]  
            return 0
        elif CheckKing(game,turn,r_move,c_move,enemy_kingpos,event) == '+':
            print(f'\nIllegal move. There is an enemy piece checking that spot.')
            game[r_piece][c_piece] = turn_pieces[turn][-2]  
            return 0
        game[r_piece][c_piece] = turn_pieces[turn][-2]
...
def IsPinned(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,event):
        current_piece = game[r_piece][c_piece]
        new_spot = game[r_move][c_move]
        game[r_piece][c_piece] = '-'
        game[r_move][c_move] = current_piece
        if CheckRookQueen(game,turn,current_kingpos[0],current_kingpos[1],event) == '+':
            print(f'\nIllegal move. The piece is pinned.')
            game[r_piece][c_piece] = current_piece
            game[r_move][c_move] = new_spot
            return 1
        elif CheckKnight(game,turn,current_kingpos[0],current_kingpos[1],event) == '+':
            print(f'\nIllegal move. The piece is pinned.')
            game[r_piece][c_piece] = current_piece
            game[r_move][c_move] = new_spot
            return 1
        elif CheckBishopQueen(game,turn,current_kingpos[0],current_kingpos[1],event) == '+':
            print(f'\nIllegal move. The piece is pinned.')
            game[r_piece][c_piece] = current_piece
            game[r_move][c_move] = new_spot
            return 1
        elif CheckPawn(game,turn,current_kingpos[0],current_kingpos[1],event) == '+':
            print(f'\nIllegal move. The piece is pinned.')
            game[r_piece][c_piece] = current_piece
            game[r_move][c_move] = new_spot
            return 1
...
def RookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event):
    rook1, rook2 = ['a1','a8'], ['h1','h8']
    if LegalRookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
        if game[squares[rook1[turn]][0]][squares[rook1[turn]][1]] != turn_pieces[turn][0]:
            ...
            rook2moves[turn] = 1
    return turn, rook1moves, rook2moves, event

def KnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event):
    if LegalKnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
    return turn, event

def BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event):
    if LegalBishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,movesplayed) == 1:
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
    return turn, event

def QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event):
    if (r_move - r_piece == 0 and c_move - c_piece != 0) or (r_move - r_piece != 0 and c_move - c_piece == 0):
        turn, rook1moves, rook2moves, event = RookMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event)
    elif abs(r_move - r_piece) == abs(c_move - c_piece):
        turn, event = BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
    else:
        print(f'\nIllegal move. The {all_pieces[piece[0]]} cannot move like that.')
    return turn, event

def KingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,kingmoves,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event):
    side, rook1, rook2 = [7,0], ['a1','a8'], ['h1','h8']
    if r_move - r_piece >= -1 and r_move - r_piece <= 1 and c_move - c_piece >= -1 and c_move - c_piece <= 1:
        if LegalKingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,event) == 0:
            return turn, kingmoves, event
        kingmoves[turn] = 1
        turn, event = MovePiece(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
    if game[side[turn]][4] == all_pieces_flipped['King'][turn]:
        if r_move - r_piece == 0 and c_move - c_piece == 2 and event != '+':
            if LegalKingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,event) == 0:
                return turn, kingmoves, event
            turn,event = ShortCastles(game,turn,piece,r_piece,c_piece,move,r_move,c_move,side,kingmoves,rook2moves,enemy_kingpos,movesplayed,event)
        elif r_move - r_piece == 0 and c_move - c_piece == -2 and event != '+':
            if LegalKingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,event) == 0:
                return turn, kingmoves, event
            turn,event = LongCastles(game,turn,piece,r_piece,c_piece,move,r_move,c_move,side,kingmoves,rook1moves,enemy_kingpos,movesplayed,event)
        elif r_move - r_piece == 0 and abs(c_move - c_piece == -2) and event == '+':
            print(f'\nIllegal move. The King cannot castle when in check.')
    else:
        print(f'\nIllegal move. The {all_pieces[piece[0]]} cannot move like that.')
    return turn, kingmoves, event

def CheckRookQueen(game,turn,r_king,c_king,event):
    a = 0
    for i in range(c_king-1,-1,-1):
        if game[r_king][c_king-1-a] in turn_pieces[turn]:
            break
        elif game[r_king][c_king-1-a] in turn_pieces[1-turn] and game[r_king][c_king-1-a] != turn_pieces[1-turn][0] and game[r_king][c_king-1-a] != turn_pieces[1-turn][3]:
            break
        elif game[r_king][c_king-1-a] == turn_pieces[1-turn][0] or game[r_king][c_king-1-a] == turn_pieces[1-turn][3]:
            return '+'
        a += 1
    a = 0
    for i in range(c_king+1,8):
        if game[r_king][c_king+1+a] in turn_pieces[turn]:
            break
        elif game[r_king][c_king+1+a] in turn_pieces[1-turn] and game[r_king][c_king+1+a] != turn_pieces[1-turn][0] and game[r_king][c_king+1+a] != turn_pieces[1-turn][3]:
            break
        ...
        a += 1
    a = 0
    for i in range(r_king-1,-1,-1):
        if game[r_king-1-a][c_king] in turn_pieces[turn]:
            break
        elif game[r_king-1-a][c_king] in turn_pieces[1-turn] and game[r_king-1-a][c_king] != turn_pieces[1-turn][0] and game[r_king-1-a][c_king] != turn_pieces[1-turn][3]:
            break
        elif game[r_king-1-a][c_king] == turn_pieces[1-turn][0] or game[r_king-1-a][c_king] == turn_pieces[1-turn][3]:
            return '+'
        a += 1
    a = 0
    for i in range(r_king+1,8):
        if game[r_king+1+a][c_king] in turn_pieces[turn]:
            break
        elif game[r_king+1+a][c_king] in turn_pieces[1-turn] and game[r_king+1+a][c_king] != turn_pieces[1-turn][0] and game[r_king+1+a][c_king] != turn_pieces[1-turn][3]:
            break
...
def CheckKnight(game,turn,r_king,c_king,event):
    sign_changes = [[1,1],[-1,1],[1,-1],[-1,-1]]
    a = [1,2]
    for i in range(8):
        if r_king + a[i//4]*(sign_changes[i%4][0]) not in range(0,8) or c_king + a[(7-i)//4]*(sign_changes[i%4][1]) not in range(0,8):
            continue
        elif game[r_king + a[i//4]*(sign_changes[i%4][0])][c_king + a[(7-i)//4]*(sign_changes[i%4][1])] == '-':
            continue
        elif game[r_king + a[i//4]*(sign_changes[i%4][0])][c_king + a[(7-i)//4]*(sign_changes[i%4][1])] == turn_pieces[1-turn][1]:
            return '+'
    return 'none'

def CheckBishopQueen(game,turn,r_king,c_king,event):
    a = 0
    d_king = max(r_king,c_king)
    for i in range(d_king-1,-1,-1):
        if r_king-1-a not in range(0,8) or c_king-1-a not in range(0,8):
            break
        elif game[r_king-1-a][c_king-1-a] in turn_pieces[turn]:
            break
        elif game[r_king-1-a][c_king-1-a] in turn_pieces[1-turn] and game[r_king-1-a][c_king-1-a] != turn_pieces[1-turn][2] and game[r_king-1-a][c_king-1-a] != turn_pieces[1-turn][3]:
            break
        elif game[r_king-1-a][c_king-1-a] == turn_pieces[1-turn][2] or game[r_king-1-a][c_king-1-a] == turn_pieces[1-turn][3]:
            return '+'
        a += 1
    a = 0
    for i in range(1,d_king+1):
        if r_king-1-a not in range(0,8) or c_king+1+a not in range(0,8):
            break
        elif game[r_king-1-a][c_king+1+a] in turn_pieces[turn]:
            break
...
def CheckPawn(game,turn,r_king,c_king,event):
    side = [1,-1]
    if r_king*side[turn] < (2+3*turn)*side[turn]:
        return 'none'
    elif c_king == 0 and game[r_king+side[1-turn]][c_king+1] == turn_pieces[1-turn][-1]:
        return '+'
    elif c_king == 7 and game[r_king+side[1-turn]][c_king-1] == turn_pieces[1-turn][-1]:
        return '+'
    elif 0 < c_king < 7 and (game[r_king+side[1-turn]][c_king-1] == turn_pieces[1-turn][-1] or game[r_king+side[1-turn]][c_king+1] == turn_pieces[1-turn][-1]):
        return '+'
    return 'none'

def CheckKing(game,turn,r_king,c_king,enemy_kingpos,event):
    for i in range(8):
        if enemy_kingpos[0] not in range(8) or enemy_kingpos[1] not in range(8):
            continue
        if abs(r_king-enemy_kingpos[0]) <= 1 and abs(c_king-enemy_kingpos[1]) <= 1:
            return '+'
    return 'none'

def CheckEnemyKing(game,turn,piece,r_piece,c_piece,move,r_move,c_move,enemy_kingpos,movesplayed,event):
    r_king, c_king = enemy_kingpos[0], enemy_kingpos[1]
    if CheckRookQueen(game,turn,r_king,c_king,event) == '+':
        return '+'
    elif CheckKnight(game,turn,r_king,c_king,event) == '+':
        return '+'
    elif CheckBishopQueen(game,turn,r_king,c_king,event) == '+':
        return '+'
    elif CheckPawn(game,turn,r_king,c_king,event) == '+':
        return '+'
    return 'none'
...
##            ['-','-','-','R','-','-','-','-'],
##            ['R','-','-','-','K','-','-','R']]
    turn, en_passant, ep_square, movesplayed, event, legalmoves =  BasicItems()
    kingmoves, rook1moves, rook2moves, current_kingpos, enemy_kingpos = TurnItems()
    while True:
        print(f'\nMoves Played: {movesplayed}\n')
        BoardPrint(game)
        GetKingPosition(game,turn,current_kingpos,enemy_kingpos)
        ...
        if len(piece) == 2: # ------------- PAWN MOVEMENT ------------- #
                ...
                if game[r_piece][c_piece] not in turn_pieces[turn]:
                    ...
                    continue
                turn, en_passant, ep_square, event = PawnMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,en_passant,ep_square,current_kingpos,enemy_kingpos,movesplayed,event)
            ...
            if piece[0] not in turn_pieces[turn]:
                ...
                turn,event = KnightMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][2]: # ------------- BISHOP MOVEMENT ------------- #
                turn,event = BishopMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,current_kingpos,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][3]: # ------------- QUEEN MOVEMENT ------------- #
                turn,event = QueenMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event)
            elif piece[0] == turn_pieces[turn][4]: # ------------- KING MOVEMENT ------------- #
                turn,kingmoves,event = KingMovement(game,turn,piece,r_piece,c_piece,move,r_move,c_move,kingmoves,rook1moves,rook2moves,current_kingpos,enemy_kingpos,movesplayed,event)
